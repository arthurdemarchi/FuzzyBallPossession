...
class Reader:
    ...
    def set_path(self, path):
        if not path[0] == '.':
            raise ValueError('Error: New path must be relative, syntax \'./data/path/to/file\'.')
        if not path[0:7] == './data/':
            raise ValueError('Error: New path can\'t be from outside data, syntax \'.data/path/to/file\'.')
        self._path = path
    ...
    def __get_list_of_files(self, directory):
        ...
        for path in list_of_paths:
            ...
            file_path = directory + path 
            if isdir(file_path):
                list_of_files = list_of_files + self.__get_list_of_files(file_path)
            else:
    ...
    def __read_from_path(self, file_path):
        ...
        distance = ((data['player_x'] - data['ball_x'])**2 + (data['player_y'] - data['ball_y'])**2)**0.5
        relative_speed = ((data['player_vx'] - data['ball_vx'])**2 + (data['player_vy'] - data['ball_vy'])**2)**0.5
